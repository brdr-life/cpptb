...
class VerilatorBackend:
    ...
    def build(
        ...
        *,
        ...
        # A stale object directory is worse than a slow one: Verilator's
        # sub-make rebuilds on source changes only, so an object compiled
        # under different defines (a timing_backend or deferred_writes flip
        # changes -D flags, not generated bytes) is silently reused and
        # fails at link -- or worse, links against the wrong semantics.
        # The fingerprint already covers every input, so reaching this
        # point means something changed; start the simulator build clean.
        if spec.object_dir.is_dir():
            shutil.rmtree(spec.object_dir)
