...
| C8 | **Generated headers are not clang-clean at Ibex scale**, two ways: DUT parameters wider than 64 bits are emitted as `int64_t` decimal literals — a ~157-bit `StatePerm` at `core_ibex_cpptb_dut.hpp:53636`, 13 sites — which clang hard-errors on and GCC quietly tolerates (a silent-truncation hazard independent of compiler); and the generated dispatch if-chains are deep enough to segfault clang 18's parser (`ParseIfStatement` recursion). Small examples are clang-clean — CI's macOS leg builds everything with Apple Clang — the failure needs Ibex-scale generated headers, which CI never builds. | `tools/codegen/cpptb_codegen/generate_dpi_bindings.py`; reproduce with `CXX=clang++` on `ports/core_ibex_cpptb` | Cross-compiler soak of the access-set parity gate |
