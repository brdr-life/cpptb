...
#include "experiments/cocotb_cpp_comparison/cpptb/apb_event_bench.hpp"
...
int main(int argc, char** argv) {
    ...
    cpptb::benchmarks::apb_event::BenchConfig config{
        env_u32("CPPTB_BENCH_ITERS", 1000)};
    cpptb::benchmarks::apb_event::BenchResult result;

    update_time(0);
    set_clocks(top.get(), 0);

    const auto start = std::chrono::steady_clock::now();
    cpptb::benchmarks::apb_event::register_benchmark(testbench, dut, config, result);
    settle(top.get());

    const uint64_t max_cycles = static_cast<uint64_t>(config.iterations) * 160u + 1000u;
    uint64_t cycles = 0;
    ...
    if (!testbench.done()) {
        std::fprintf(stderr, "cpptb: timed out after %llu cycles\n",
                     static_cast<unsigned long long>(cycles));
        return 1;
    }

    top->final();
    const auto status = cpptb::benchmarks::apb_event::done(result, dut);

    std::printf(
        "CPPTB_BENCH_RESULT iterations=%u checks=%llu sim_cycles=%llu wall_ms=%.3f failures=%u\n",
