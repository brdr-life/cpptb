...
The `mojotb/` prototype proves the first slice of that architecture with
Verilator and VPI:

```text
mojotb/examples/vpi_counter.sv
  -> verilator --cc --vpi --public-flat-rw
  -> mojotb/verilator_vpi_host.cpp
  -> dlopen(build/mojotb/libcounter_test.dylib)
  -> mojotb/tests/counter_test.mojo
```
