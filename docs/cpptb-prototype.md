...
- `cpptb/runtime.hpp`: primitive signal, trigger, wait request, scheduler, and
  DUT handle wrappers.
- `cpptb/tests/counter_test.cpp`: reset driver, enable driver, count monitor,
  and final check.
- `cpptb/verilator_vpi_host.cpp`: Verilator host and VPI callback scheduler.
- `cpptb/examples/vpi_counter.sv`: local copy of the counter RTL for this
  experiment.
...
RgGen-derived DUT under `cpptb/rggen_apb_event/`.
