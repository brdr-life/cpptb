...
docs-check: docs-diagram-check docs-api-name-check docs-build
...
# The hand-curated library reference must keep naming things that exist:
# every ###-heading API name is required to appear in the headers the page
# declares. Rename in the header, fail in the docs build.
docs-api-name-check:
	python3 tools/docs/check_api_names.py

# The api_links extension hash rides along as a config override so that
# editing the extension invalidates Sphinx's cached doctrees; without it a
# linker change silently leaves stale pages.
docs-sphinx-build:
	$(DOCS_RUN) sphinx-build -W --keep-going -c tools/docs/sphinx \
		-d $(DOCS_BUILD_DIR)/sphinx-doctrees \
		-D api_links_source_hash=$$(sha256sum tools/docs/sphinx/_ext/api_links.py | cut -d' ' -f1) \
