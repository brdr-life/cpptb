...
`ifdef CPPTB_SV_DPI_CALENDAR_TIMING
  task automatic calendar_initialize();
    ...
    calendar_clock_next_edge[1] = $time + time'(1ns) + calendar_clock_half_period[1];
  endtask
  ...
  task automatic calendar_drive_due_clocks();
    ...
    if (calendar_clock_active[0] &&
      ...
      if ((event_edge == EDGE_RISING) ||
        ...
        run_step(PHASE_EDGE, SIGNAL_WRITECLK, event_edge, requests);
        service_requests(requests);
      end
    end
    if (calendar_clock_active[1] &&
      ...
      if ((event_edge == EDGE_RISING) ||
          track_falling_edges) begin
        run_step(PHASE_EDGE, SIGNAL_READCLK, event_edge, requests);
        service_requests(requests);
      end
    end
  endtask
...
`endif
  ...
  task automatic cpptb_dual_clock_mailbox_dpi_phase_dispatch(
      input int unsigned phase
  );
    int requests;
    ...
    service_requests(requests);
  endtask
  export "DPI-C" task cpptb_dual_clock_mailbox_dpi_phase_dispatch;
