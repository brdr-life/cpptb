...
module dpi_stream_fifo;
  ...
  import "DPI-C" context function void cpptb_stream_fifo_dpi_init(
      input int unsigned iterations,
      input longint unsigned timeprecision_fs
  );
  import "DPI-C" context function int cpptb_stream_fifo_dpi_step(
  ...
  import "DPI-C" function void cpptb_stream_fifo_dpi_pull_outputs(
      output int unsigned out_words[]
  );
  import "DPI-C" function longint unsigned cpptb_stream_fifo_dpi_next_timer_deadline();
  import "DPI-C" function int unsigned cpptb_stream_fifo_dpi_edge_interest(
...
`endif
  ...
  task automatic run_step(
    ...
    pack_inputs();
    requests = cpptb_stream_fifo_dpi_step(phase, $time, sim_cycles,
                               event_signal_id, event_edge,
                               in_words);
    if ((requests >= 0) &&
        ((phase == PHASE_INIT) ||
         ((requests & STEP_OUTPUTS_CHANGED) != 0))) begin
      cpptb_stream_fifo_dpi_pull_outputs(out_words);
      apply_outputs();
    end
...
`endif
  ...
  task automatic update_timer_schedule();
    ...
    deadline = cpptb_stream_fifo_dpi_next_timer_deadline();
...
`endif
  ...
  task automatic cpptb_stream_fifo_dpi_phase_dispatch(
      input int unsigned phase
  );
    int requests;
    ...
    service_requests(requests);
  endtask
  export "DPI-C" task cpptb_stream_fifo_dpi_phase_dispatch;
...
`endif
    ...
    cpptb_stream_fifo_dpi_init(iterations, TIMEPRECISION_FS);
`ifdef CPPTB_SV_DPI_CALENDAR_TIMING
    calendar_initialize();
`endif
