...
module dpi_stalling_responder;
  ...
  localparam int SIGNAL_REQUEST = 2;
  localparam int SIGNAL_STALL = 3;
  localparam int SIGNAL_LATENCY = 4;
  localparam int SIGNAL_REQUESTDATA = 5;
  localparam int SIGNAL_RESPONSEVALID = 6;
  localparam int SIGNAL_RESPONSEDATA = 7;
  localparam int SIGNAL_BUSY = 8;
  localparam int INPUT_SIGNAL_CLK = 0;
  localparam int INPUT_SIGNAL_RESPONSEVALID = 1;
  localparam int INPUT_SIGNAL_RESPONSEDATA = 2;
  localparam int INPUT_SIGNAL_BUSY = 3;
  localparam int OUTPUT_SIGNAL_RSTN = 0;
  localparam int OUTPUT_SIGNAL_REQUEST = 1;
  localparam int OUTPUT_SIGNAL_STALL = 2;
  localparam int OUTPUT_SIGNAL_LATENCY = 3;
  localparam int OUTPUT_SIGNAL_REQUESTDATA = 4;
  localparam int CPPTB_SIGNAL_COUNT = 9;
  localparam int INPUT_WORD_COUNT = 4;
  localparam int OUTPUT_WORD_COUNT = 5;

  import "DPI-C" context function void cpptb_stalling_responder_dpi_init(
      input int unsigned iterations,
      input longint unsigned timeprecision_fs
  );
  import "DPI-C" context function int cpptb_stalling_responder_dpi_step(
  ...
  import "DPI-C" function void cpptb_stalling_responder_dpi_pull_outputs(
      output int unsigned out_words[]
  );
  import "DPI-C" function longint unsigned cpptb_stalling_responder_dpi_next_timer_deadline();
  import "DPI-C" function int unsigned cpptb_stalling_responder_dpi_edge_interest(
...
`endif
  ...
  task automatic pack_inputs();
    ...
    in_words[INPUT_SIGNAL_RESPONSEVALID] = response_valid;
    in_words[INPUT_SIGNAL_RESPONSEDATA] = response_data;
    in_words[INPUT_SIGNAL_BUSY] = busy;
  endtask

  task automatic apply_outputs();
    rst_n = out_words[OUTPUT_SIGNAL_RSTN][0];
    request = out_words[OUTPUT_SIGNAL_REQUEST][0];
    stall = out_words[OUTPUT_SIGNAL_STALL][0];
    latency = out_words[OUTPUT_SIGNAL_LATENCY][7:0];
    request_data = out_words[OUTPUT_SIGNAL_REQUESTDATA];
  endtask
...
`endif
  ...
  task automatic run_step(
    ...
    pack_inputs();
    requests = cpptb_stalling_responder_dpi_step(phase, $time, sim_cycles,
                               event_signal_id, event_edge,
                               in_words);
    if ((requests >= 0) &&
        ((phase == PHASE_INIT) ||
         ((requests & STEP_OUTPUTS_CHANGED) != 0))) begin
      cpptb_stalling_responder_dpi_pull_outputs(out_words);
      apply_outputs();
    end
...
`endif
  ...
  task automatic update_timer_schedule();
    ...
    deadline = cpptb_stalling_responder_dpi_next_timer_deadline();
...
`endif
  ...
  task automatic service_requests(
    ...
    if ((requests & STEP_EDGE_INTEREST_CHANGED) != 0) begin
      edge_interest[SIGNAL_RESPONSEVALID] = cpptb_stalling_responder_dpi_edge_interest(SIGNAL_RESPONSEVALID);
    end
...
`endif
  ...
  task automatic cpptb_stalling_responder_dpi_phase_dispatch(
      input int unsigned phase
  );
    int requests;
    ...
    service_requests(requests);
  endtask
  export "DPI-C" task cpptb_stalling_responder_dpi_phase_dispatch;
...
`endif
      if (status == 0) begin
        event_edge = response_valid ? EDGE_RISING : EDGE_FALLING;
        if (
            ((event_edge == EDGE_RISING) && ((edge_interest[SIGNAL_RESPONSEVALID] & 1) != 0)) ||
            ((event_edge == EDGE_FALLING) && ((edge_interest[SIGNAL_RESPONSEVALID] & 2) != 0))) begin
          run_step(PHASE_EDGE, SIGNAL_RESPONSEVALID, event_edge, requests);
          service_requests(requests);
`ifdef CPPTB_SV_DPI_CALENDAR_TIMING
          calendar_drain_phases();
`endif
...
`endif
    ...
    cpptb_stalling_responder_dpi_init(iterations, TIMEPRECISION_FS);
`ifdef CPPTB_SV_DPI_CALENDAR_TIMING
    calendar_initialize();
`endif
...
`endif
    ...
    timer_generation++;
    if (status < 0) begin
      $fatal(1, "stalling_responder DPI testbench failed");
    end
    $finish;
  end
...
endmodule : dpi_stalling_responder
