#include "benchmarks/peripheral_suite/testbenches/cpp_dpi/framework/peripheral_suite_fixture.hpp"
...
void launch(coro::Testbench& scheduler, coro::Task<void> task) {
...
coro::Task<void> ApbMaster::write(uint32_t byte_addr, uint32_t data) const {
...
coro::Task<void> ApbMaster::read_expect(uint32_t byte_addr,
...
coro::Task<void> ApbMaster::read_ok(uint32_t byte_addr) const {
    co_await FallingEdge{clock_};
    bus_.PADDR.set(byte_addr & 0xfff);
    bus_.PWDATA.set(0);
    bus_.PWRITE.set(0);
...
coro::Task<void> run_sequence(PeripheralTb tb, PeripheralTb::Process process) {
    co_await process(tb);
    tb.end_sequence();
}
...
coro::Task<void> PeripheralTb::wait_cycles(uint32_t cycles) const {
    for (uint32_t i = 0; i < cycles; ++i) {
        co_await RisingEdge{dut.HCLK};
    }
}

coro::Task<void> PeripheralTb::wait_reset_released() const {
    while (dut.HRESETn.get() == 0) {
        co_await RisingEdge{dut.HCLK};
    }
}

coro::Task<void> PeripheralTb::wait_timer_irq(uint32_t bit,
