...
from benchmarks.framework_comparison.open_cores.workload import (  # noqa: E402
    DEFAULT_ITERATIONS,
    DESCRIPTIONS,
    MODES,
    WORKLOADS,
        ...
        for token in match.group("fields").split():
            ...
            try:
                ...
                try:
                    ...
                    result[key] = value
        return result
    raise BenchmarkError("output has no OPEN_CORE_BENCH_RESULT line")
...
def command_for(mode: str, workload: str, iterations: int):
    plusarg = f"+OPEN_CORE_BENCH_ITERS={iterations}"
    if mode == "pure_sv":
        return [BUILD_DIR / f"pure_sv_{workload}" / "Vopen_cores_sv_tb", plusarg], None
    if mode == "cpp_dpi":
        return [BUILD_DIR / f"cpp_dpi_{workload}" / "Vdpi_open_cores_benchmark", plusarg], None
    if mode == "cpp_vpi":
        env = os.environ.copy()
        env["OPEN_CORE_BENCH_ITERS"] = str(iterations)
        return [BUILD_DIR / f"cpp_vpi_{workload}" / "Vopen_cores_benchmark_top"], env
    if mode == "cocotb":
        env = os.environ.copy()
...
def compare(workload: str, iterations: int, runs: int, journal) -> dict:
    print(f"[{workload}] semantic preflight (1)", flush=True)
    reference = None
    preflight = {}
    for mode in MODES:
        entry = sample(mode, workload, 1, reference)
    ...
    for round_index in range(runs):
        ...
        evidence = {
            mode: tuple(
                round_entries[mode][key]
                for key in ("transactions", "checks", "sim_cycles", "checksum", "failures")
            )
            for mode in MODES
        }
    ...
    ratios = {
        f"{mode}_over_pure_sv": modes[mode]["process_wall_ms_median"] / sv_ms
        for mode in MODES
    }
...
def render_markdown(result: dict) -> str:
    lines = [
        "# Open-source core framework comparison",
        "",
    ...
    for workload in WORKLOADS:
        ...
        ratios = entry["ratios"]
        lines.append(
            f"| {entry['description']['title']} | {entry['iterations']:,} | "
            f"{entry['reference_sim_cycles']:,} "
            f"| {modes['pure_sv']['process_wall_ms_median']:.1f} ms / 1.00x "
            f"| {modes['cpp_dpi']['process_wall_ms_median']:.1f} ms / "
            f"{ratios['cpp_dpi_over_pure_sv']:.2f}x "
            f"| {modes['cpp_vpi']['process_wall_ms_median']:.1f} ms / "
            f"{ratios['cpp_vpi_over_pure_sv']:.2f}x "
            f"| {modes['cocotb']['process_wall_ms_median']:.1f} ms / "
            f"{ratios['cocotb_over_pure_sv']:.2f}x |"
        )
    guard = result["performance_guard"]
    lines.extend([
        "",
        "Runs are serialized and mode order rotates each round. Transactions, checks, simulation cycles, checksum, and failures must match across all four modes.",
        "",
        f"C++ DPI / pure-SV 1.10x guard: **{guard['status']}**. It is advisory unless `--enforce-guard` is passed.",
        "",
    ])
...
def main(argv=None):
    ...
    if not args.skip_build:
        output, _ = run_command(["make", "framework-comparison-open-cores-build"])
        if output.strip():
            print(output, end="")
    ...
    try:
        with journal_path.open("a", encoding="utf-8") as journal:
            for workload in workloads:
                ...
                result["workloads"][workload] = compare(
                    workload, iterations, args.runs, journal
                )
        ratios = {
        ...
        (RESULT_DIR / f"{args.output_stem}.json").write_text(
            json.dumps(result, indent=2, sort_keys=True) + "\n", encoding="utf-8"
        )
        if "performance_guard" in result:
            (RESULT_DIR / f"{args.output_stem}.md").write_text(
