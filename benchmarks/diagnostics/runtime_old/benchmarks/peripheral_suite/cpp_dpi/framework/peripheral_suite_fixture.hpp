...
class ApbMaster {
   public:
    ...
    coro::Task write(uint32_t byte_addr, uint32_t data) const;
    coro::Task read_expect(uint32_t byte_addr, uint32_t expected) const;
    coro::Task read_ok(uint32_t byte_addr) const;
...
class PeripheralTb {
   public:
    using Process = coro::Task (*)(PeripheralTb);
    ...
    coro::Task wait_cycles(uint32_t cycles) const;
    coro::Task wait_reset_released() const;
    coro::Task wait_timer_irq(uint32_t bit, uint32_t max_cycles) const;
   ...
   private:
    ...
    friend coro::Task run_sequence(PeripheralTb tb, Process process);
};
