#include "benchmarks/peripheral_suite/cpp_dpi/framework/peripheral_suite.hpp"
...
Task reset_dut(PeripheralTb tb) {
...
Task timer_register_sequence(PeripheralTb tb) {
...
Task spi_register_sequence(PeripheralTb tb) {
    co_await tb.wait_reset_released();
...
Task i2c_register_sequence(PeripheralTb tb) {
    co_await tb.wait_reset_released();
...
Task timer_irq_monitor(PeripheralTb tb) {
    co_await tb.wait_reset_released();
...
Task spi_handshake_monitor(PeripheralTb tb) {
    co_await tb.wait_reset_released();
    while (tb.sequences_running()) {
        co_await RisingEdge{tb.dut.HCLK};
        co_await Delay{1_ps};
...
Task i2c_pad_monitor(PeripheralTb tb) {
    co_await tb.wait_reset_released();
    while (tb.sequences_running()) {
        co_await RisingEdge{tb.dut.HCLK};
        co_await Delay{1_ps};
